...
ContactPairs:
  - Name: "Pair1"
    Type: "KinematicContact" # 四大类可选其一
    MasterPartID: 2 # [必填]
    SlavePartID: 1 # [必填]

    # --------------------------------
    # 对应 KinematicContact.cpp
    # --------------------------------
    RestitutionCoeff: 0.0 # 动能恢复（反弹）系数

    # --------------------------------
    # 对应 PenaltyContact.cpp (经典软弹簧)
    # --------------------------------
    PenaltyFactor: 1.0 # 根据体积模量算出的刚度乘子
    PenaltyStiffness: 1.0e6 # [覆写级] 直接写死弹簧刚度值
    PinballRatio: 1.1 # 接触半径/容差识别膨胀倍率

    # --------------------------------
    # 对应 ViscousPenaltyContact.cpp (带阻尼弹簧)
    # --------------------------------
    # 包含 PenaltyContact 的因子与刚度，额外追加：
    DampingCoeff: 0.5 # 切向与法向共同阻尼系数

    # --------------------------------
    # 对应 NonlinearPenaltyContact.cpp (非线性防穿模弹簧)
    # --------------------------------
    # 包含 PenaltyContact 的因子与刚度，额外追加：
    ...
    # --------------------------------
    # 对应 SillingContact.cpp (PD标准互斥点力)
    # --------------------------------
