...
LinearElasticMat::LinearElasticMat(const std::string& name) : MechanicalMaterial(name) {}
...
void LinearElasticMat::initialize(const YAML::Node& matNode) {
  ...
  LOG_INFO("[LinearElasticMat] Material '" + name_ + "' is initialized. E = " +
           std::to_string(youngsModulus_) + ", nu = " + std::to_string(poissonsRatio_) +
...
Eigen::Matrix3d LinearElasticMat::ComputeStress(const Eigen::Matrix3d& strain) const {
  // 广义胡克定律: \sigma = \lambda * tr(\varepsilon) * I + 2 * \mu * \varepsilon
  double trace = strain.trace();
  return lambda_ * trace * Eigen::Matrix3d::Identity() + 2.0 * mu_ * strain;
}

Eigen::Matrix3d LinearElasticMat::ComputePK1Stress(const Eigen::Matrix3d& F) const {
  ...
  Eigen::Matrix3d S = ComputeStress(E);
  
  // 第一类 Piola-Kirchhoff 应力 P = F * S
  return F * S;
}

void LinearElasticMat::allocateStateVariables(PDCommon::Field::FieldManager& fm) {
  // 线弹性暂无状态变量
}

size_t LinearElasticMat::getNumStateVariables() const {
  return 0;
}

void LinearElasticMat::evaluate() {
  // 预留接口：本构具体运算通过 ComputePK1Stress 暴露给 Kernel 计算，无需在此缓存状态
}
