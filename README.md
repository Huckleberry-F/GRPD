...
7. 🔮 **矩阵免组装纯隐式求解 (Matrix-Free Implicit Solvers)**
   `MatrixFreeImplicitIntegrator` 现在可在同一套 `TimeIntegrator` 注册体系中求解二阶力学平衡与一阶稳态热传导残差。求解器保持无全局刚度矩阵装配，直接对 $R(u)=0$ 进行迭代。
   - 核心支持基于 Jacobian-Free Newton-Krylov (**JFNK**) 的有限差分矩阵向量积与 GMRES 子空间求解。
   - 同步支持纯向量点积操作的 Limited-memory BFGS (**L-BFGS**) 方向更新，并在子步边界施加、残差归零、状态提交与输出时序上对齐 ADR 风格。
   - 热学路径已支持 `Solver.Type: Thermal` + `TimeIntegrator: MatrixFreeImplicit` 的稳态多载荷步对标算例，例如 `Examples/Thermal_Validation_Steady_Steps/PD.yaml`。

8. 🧱 **时间相关材料与蠕变状态变量闭环 (Creep Constitutive Models)**
   材料库新增 `CreepMaterialBase`，并通过 `FieldManager` 统一分配 `EqCreepStrain_Old/Trial` 与 `CreepStrain_Old/Trial` 状态场。当前已注册 `NortonBaileyCreepMat`、`TimeHardeningCreepMat`、`StrainHardeningCreepMat` 三类模型，统一采用 Trial/Old 交换与 `commitState()` 提交链路，避免在隐式试探内循环中污染历史状态。

9. 🧪 **Agent/MCP 验证与跨平台工程化工具链**
   `.agents/` 中已收敛为 `superpowers` + `superCAE` 技能体系和五类 MCP 服务：`grpd-server`、`ansys-server`、`grpd-validation-server`、`grpd-experiment-server`、`matlab-server`。配套 `setup_mcp.py` 可自动生成本地与 IDE MCP 配置，支撑 GRPD/ANSYS/MATLAB 对标、VTK 切片误差分析和实验记录。
...
![Friction Block Stick](Examples/Friction_Blocks_Contact/friction0.4_compressed.gif)
_原生库仑摩擦接触算法验证 (Coulomb Friction)：具有大位移接触滑动与稳定能量耗散的摩擦方块对齐推覆_

![Friction Block Slip](Examples/Friction_Blocks_Contact/slip0.4_compressed.gif)
_原生库仑摩擦接触算法验证 (Coulomb Friction)：大变形接触下精准的黏滞-滑移转态与摩擦界面捕获_
...
脚本存放在 `setup/` 物理目录下，按平台拆分为 `setup/win/`、`setup/mac/` 与 `setup/linux/`。Windows 与 macOS 已接入 `CMakePresets.json`，默认导出 `compile_commands.json`，供 clangd、CMake Tools 与 IDE 索引直接复用。
...
2. **📦 依赖补全与虚拟环境配置 (`install_deps`)**：
   ...
   - **自动注册本地与全局 IDE MCP 服务**（将 `grpd-server`、`ansys-server`、`grpd-validation-server`、`grpd-experiment-server`、`matlab-server` 等自动写入 `.agents/settings.json` 与 `~/.gemini/*/mcp_config.json`），实现开箱即用的 Agent CAE 交互能力。
3. **🛠️ 自动编译与构建测试 (`build_project`)**：自动配置 CMake，在 Release 模式下根据 CPU 物理核心数进行多线程并行编译；Windows 优先尝试 `win-ninja` preset，失败后回退 `win-mingw` preset，macOS 使用 `mac-release` preset，Linux 使用原生 CMake build 目录。
...
```bash
git clone --recurse-submodules https://github.com/Huckleberry-F/GRPD.git
cd GRPD
```

#### 💻 Windows 平台
推荐使用 PowerShell 执行平台脚本：
```powershell
powershell -ExecutionPolicy Bypass -File .\setup\win\setup_build.ps1
```

如果需要拆分执行，也可以单独运行：
```powershell
powershell -ExecutionPolicy Bypass -File .\setup\win\check_env.ps1
powershell -ExecutionPolicy Bypass -File .\setup\win\install_deps.ps1
.\setup\win\build_project.bat
```
...
cmake --build build --preset mac-release-build
```
...
进入任意包含 `PD.yaml` 的算例目录并启动可执行文件即可运行，例如：

```bash
cd Examples/Thermal_Validation_Steady_Steps
../../bin/release/GRPD
```

Windows PowerShell 下对应为：
...
计算结束后，当前目录下将自动生成形如 `Result_YYYYMMDD_HHMMSS/` 的时间戳数据包。Legacy `.vtk` 时间步会自动配套生成 `.vtk.series` 索引文件，直接在 ParaView 中打开该 series 文件即可按物理时间浏览多步结果。
...
graph TD
    ...
    subgraph EngineSys ["⚙️ 引擎控制与积分层 (Src)"]
        ...
        TimeIntegrator -.->|多态挂载| Integrators["ADR / MatrixFreeImplicit / CentralDifference / ExplicitEuler"]
    end
...
classDiagram
    ...
    class MatrixFreeImplicitIntegrator {
        -algorithm_
        +solveJFNK()
        +solveLBFGS()
    }
...
| ⏱️ **演化积分控制<br>(Time Integrator)**     | `ExplicitEuler`<br>`CentralDifference`<br>`ADR`<br>`StaggeredIntegrator`<br>`MatrixFreeImplicit` | 显式前向欧拉（一阶热扩散）、显式二阶中心差分（高速冲击）、自适应动态弛豫（静力平衡态）、多场交错步耦合控制，以及 JFNK/L-BFGS 无矩阵隐式稳态求解。 |
| ⚙️ **空间计算核<br>(PDKernel)**              | `NOSB_M`<br>`NOSB_T`                                                         | 力学非常规态基（处理大变形/大旋转客观应力率）、热传导态基（无网格多维连续热通量演化）。                                 |
| 🧱 **材料本构库<br>(Material)**              | `LinearElastic`<br>`J2Plasticity`<br>`JCPlasticity`<br>`J2VoceLemaitre`<br>`FourierThermal`<br>`NortonBaileyCreepMat`<br>`TimeHardeningCreepMat`<br>`StrainHardeningCreepMat` | 圣维南线弹性介质、J2 Von-Mises 流动塑性径向返回、Johnson-Cook 强冲击本构、Voce-Lemaitre 损伤塑性、傅里叶导热介质，以及三类时间相关蠕变模型。 |
...
| 🧪 **验证与 Agent 工具链<br>(MCP Services)** | `grpd-server`<br>`ansys-server`<br>`grpd-validation-server`<br>`grpd-experiment-server`<br>`matlab-server` | 通过 `.agents/mcp/` 对 GRPD 运行、ANSYS 基准解、VTK/TXT 误差对比、SQLite 实验记录、MATLAB 单点本构验证进行服务化封装。 |
...
```text
General-Peridynamics/
├── .agents/           # Agent/SuperCAE 技能、MCP 服务与自动配置脚本
│   ├── mcp/           # → grpd/ansys/validation/experiment/matlab 五类本地 MCP 服务
│   ├── skills/        # → superpowers 通用流程与 superCAE 专项工程规则
│   └── setup_mcp.py   # → 自动生成本地与 IDE MCP 配置
...
│   ├── Box_Heat/      # → 均质长方体空间导热 Benchmark
│   ├── Thermal_Validation_5x10/         # → GRPD/ANSYS 热传导对标算例
│   ├── Thermal_Validation_Steady_Steps/ # → MatrixFreeImplicit 稳态多载荷步热算例
│   ├── Sphere/        # → 标准球体径向扩展 Benchmark
│   ├── bone/          # → 特异性曲面网格 (如骨骼) 分析 Benchmark
│   └── Engine/        # → 复杂装配体级仿真案例
├── setup/             # Windows/macOS/Linux 原生环境检测、依赖安装与构建脚本
│   ├── win/
│   ├── mac/
│   └── linux/
├── CMakePresets.json  # Windows/macOS 常用 CMake preset 与 compile_commands.json 导出
...
经过严格精简的代码库架构，剔除所有第三方冗余依赖，核心引擎及测试基准代码规模（截至当前，统计 `Src`、`PDCommon`、`Examples`、`Generate_py` 下 C++/Python 源文件）如下：

- **统计文件数**：**218** 个
- **有效代码行数（包含代码与纯注释）**：**18,367** 行
- **纯代码行数（排除纯注释与空行）**：**13,607** 行
- **纯注释行数**：4,760 行
- **空行数**：3,841 行
- **物理总行数**：22,208 行

_(注：统计范围包含 `Src`, `PDCommon`, `Examples`, `Generate_py` 等核心 C++ 源文件与 Python 预处理脚本，已排除 `Third` 库、`.agents` 服务代码、构建产物和算例数据文件)_

---

## 📌 版本更新日志 (Changelog)

### v5.0 — 跨平台工程化、MCP 验证链路与稳态隐式热求解 (Cross-Platform Tooling, MCP Validation & Steady Implicit Thermal)

- **矩阵免组装稳态热传导闭环 (Matrix-Free Steady Thermal Closure)**：`MatrixFreeImplicitIntegrator` 已扩展至一阶热传导系统，可通过 `Solver.Type: Thermal`、`Kernel: NOSB_T` 与 `TimeIntegrator: MatrixFreeImplicit` 直接求解稳态残差。该路径在每个隐式子步内先施加 Dirichlet 约束，再计算热通量残差，并对恒温边界自由度执行残差归零，防止固定温度点污染全局收敛范数。
- **JFNK/L-BFGS 多载荷步输出对齐 (ADR-Style Logging & Output Alignment)**：隐式积分器的载荷步、子步、`KBC` Ramp/Step、物理时间输出与 `ADR_Integrator` 语义对齐。收敛子步结束后统一执行 `FieldManager::executeAllRegisteredSwaps()` 与材料 `commitState()`，随后再次施加边界锁定并输出 VTK，避免输出场与提交状态错位。
- **局部载荷因子修复 (Local activeLF Scaling Fix)**：修复多载荷步隐式求解中 `activeLF` 重复叠加造成的载荷跳变问题，使每个 LoadStep 内部的 Ramp 因子只描述当前子步进度，物理时间由相邻 LoadStep 的 `targetTime` 插值确定。
- **蠕变本构族落地 (Creep Constitutive Family)**：新增 `CreepMaterialBase` 及 `NortonBaileyCreepMat`、`TimeHardeningCreepMat`、`StrainHardeningCreepMat`。材料状态通过 `EqCreepStrain_Old/Trial` 与 `CreepStrain_Old/Trial` 管理，蠕变增量由等效应力、当前时间与历史等效蠕变应变控制，遵守 GRPD 的 Trial/Old/Commit 状态冻结纪律。
- **MCP 微服务化验证链路标准化 (Standardized MCP Validation Services)**：`.agents/mcp/` 中形成 `grpd-server`、`ansys-server`、`grpd-validation-server`、`grpd-experiment-server`、`matlab-server` 的服务化布局，支持 GRPD 编译运行、ANSYS APDL 基准、MATLAB 单点本构校验、VTK/TXT 切片对齐、SQLite 实验记录与误差报告生成。
- **ANSYS/GRPD 对标鲁棒性增强 (ANSYS-GRPD Alignment Fixes)**：验证链路新增物理时间对齐、GRPD solver mapping、ANSYS 子步后缀解析、边界点一维线性外推、3D 对角线投影与多通道 component mapping，减少热传导对标中路径采样和结果文件命名带来的非物理误差。
- **ParaView 时间序列输出修复 (Legacy VTK Series Output)**：由于 `.pvd` 仅适配 VTK XML 格式，Legacy `.vtk` 输出改为自动生成 `.vtk.series` JSON 索引。用户可直接打开 series 文件浏览多时间步，避免 ParaView 对 legacy 文件组识别不稳定。
- **跨平台环境和 Windows 开发体验升级 (Cross-Platform Setup & Windows DX)**：新增 `setup/win`、`setup/mac`、`setup/linux` 分平台脚本和 `CMakePresets.json`。Windows 构建优先使用 Ninja preset，失败后回退 MinGW preset；`.clangd` 依赖 `build/compile_commands.json` 并处理 MinGW rsp 文件问题，显著降低 clangd include 误报与 PowerShell 编码/路径噪声。
- **仓库清理与文档重组 (Repository Cleanup)**：清理废弃 `tools/`、Doxygen 生成物和旧脚本归档，将长期维护入口收束到 `docs/`、`.agents/`、`setup/` 与示例算例目录，降低主仓库噪声。

### v4.9 — 轴对称非局部算子与隐式损伤绝对稳定化 (Axisymmetric Operator & Absolute Damage Stabilization)

- **轴对称非局部算子与全尺寸架构 (Axisymmetric Non-local Operator)**：新增 `NOSB_Axis` 积分核，完美支持 2D 轴对称结构的弹塑性与断裂模拟。针对中心轴 $R \to 0$ 的奇异性，引入了基于洛必达法则的环向应变投射与附加通量修正。支持全尺寸或半对称结构的快速切换验证。
- **隐式损伤演化积分的解析稳定化 (Absolute Analytical Stabilization for Implicit Damage)**：彻底重构了 `J2VoceLemaitreMat` 中的 Lemaitre 隐式损伤积分算法。将极易因驱动力激增而发生 `NaN` 崩溃的牛顿-拉夫逊 (Newton-Raphson) 迭代循环，替换为基于稳态方程 $W^2 - W_n W + Y_{term} = 0$ 的精确一元二次解析解，并在判别式 $\Delta < 0$ 时引入严格的物理破断截断，彻底根除了深层断裂时的 `KineticRatio: nan` 奇异性，使局部失效计算达到无条件稳定。
- **分步对称加载边界支持 (Multi-step Symmetry Boundary)**：完善并测试了对双端受拉、带缺陷构件的多阶段 `LoadSteps` 控制流配置，顺利跑通了经典的圆柱缺口杆 (Cylindrical Notched Bar) 塑性损伤失效算例。


### v4.8 — 增量 Modified Newton-Raphson 架构与双模 ADR 融合 (Incremental Modified NR & Dual-Mode ADR)

- **增量修正力驱动的 Modified NR 架构 (forceCorr-Driven Incremental NR)**：彻底重构了 `ADR_Integrator` 的外层 NR 迭代逻辑。引入本构修正力场 `forceCorr_`，在每轮 NR 迭代开始时计算真实非线性加速度与冻结弹性加速度的差值 $F_{corr} = F_{int}^{frozen}(u_k) - F_{int}^{true}(u_k)$，并在内层 ADR 每个微步中叠加到冻结态加速度上。数学上将 ADR 的稳态方程从求解总位移 $F_{ext} = K_0 u$ 变换为求解残差增量 $K_0 \Delta u = R_{true}(u_k)$，实现了无矩阵（Matrix-Free）的标准 Modified Newton-Raphson 迭代。

  **新旧方法的数学本质对比**：

  | | 旧方法 (Picard 逐次替代法) | 新方法 (Modified NR 残差增量法) |
  |---|---|---|
  | **更新通项** | $u_{k+1} = K_0^{-1}(F_{ext} + K_0 \varepsilon_p^{equiv}(u_k))$ | $u_{k+1} = u_k + K_0^{-1} R_{true}(u_k)$ |
  | **内循环求解目标** | 总位移（绝对坐标） | 增量（残差修正量） |
  | **冻结态塑性基底** | 每轮切换（`pSOld_` → `pSTrial_`） | 始终固定（`pSOld_`） |
  | **$u_k$ 在公式中的角色** | 仅通过 $\varepsilon_p^{trial}$ 间接影响 | 直接出现在更新公式中 |

- **双模 ADR 运行体系 (Dual-Mode ADR Architecture)**：通过 YAML 配置 `NRStateFrozen` 一键切换两种求解模式：
  1. `NRStateFrozen: true`：增量 Modified NR 模式，内层 ADR 在冻结弹性刚度下求解残差驱动增量，外层通过解冻本构校验宏观收敛。
  2. `NRStateFrozen: false`：纯显式 ADR 模式，内层直接使用真实非线性本构硬算至稳态，内循环收敛即代表真实平衡，跳过外层 NR 校验。

- **stateMode 关键修复 (Critical stateMode Fix)**：修复了冻结态力评估时 `outerIter > 0` 导致 `stateMode=2` 读取被解冻评估污染的 `pSTrial_` 的致命 Bug。强制冻结态始终使用 `outerIter=0`（`stateMode=1`），确保 $F_{corr}$ 代表真实的弹塑性偏差而非零值。
...
- **五重自适应断裂判定策略 (Quintuple Fracture Strategies)**: 全面梳理并扩充了 5 种高度解耦的断裂评估策略，满足从理想准静态到高爆冲击的全场景覆盖。深度对标了工业界经典的断裂模式（如 Abaqus/ANSYS 经典算法）以及近场动力学专有的显式耗散模式：
  1. 🌟 `StrictNRFracture` **(严格隐式断裂 - 黄金稳定标准)**：【原名 ImplicitConverged】外层隐式 NR 彻底双收敛后执行断键，若发生断键则强制重置收敛状态，带着新裂纹原地重启 NR 迭代寻找新平衡。**特征**：极度严谨、不可逆。逻辑等同于带有极强网格稳定控制的高级隐式内聚力断裂模型，最大化避免了瞬态伪震荡导致的误杀，专治裂纹尖端假性崩塌。
  2. 🧩 `LaggedNRFracture` **(滞后试探隐式断裂 - 粘性稳定化对齐)**：【原名 ImplicitState】在外层 NR 寻找平衡的内循环试探期间，允许即将断裂的键提前“软化”（Damage_Trial = 1）进入假死状态卸力，帮助 NR 快速收敛；当 NR 完全收敛时，再将这些假死的键做正式的拓扑剔除。**特征**：深度对标 Abaqus/Standard 中针对复杂断裂极难收敛而引入的**“损伤粘性正则化（Viscous Regularization / Delayed Damage）”**，极大加速了复杂裂纹网络的隐式收敛速度。
  3. ⚡ `SimpleNRFracture` **(单步隐式断裂 - 工业经典一步法)**：【**最新增设**】在内循环中完全屏蔽损伤（保持纯弹塑性抗拉），仅在 NR 完美收敛的瞬间切断超限的键，且**绝不要求重新迭代**，直接携带新伤口迈入下一个载荷步。**特征**：完美复刻 **ANSYS 的 `EKILL` 生死单元** 或 **Abaqus/Standard 的经典单元删除（Element Deletion without Cutback）** 法则。计算成本极低且极其稳定，是追求工程化分析效率的最优折中方案。
  4. 🚀 `FastInnerLoop` **(快速显式断裂 - 极速冲击防爆)**：纯显式动力学专属模式。内循环一旦收敛就斩断损伤键，并头也不回地直接进入下一个真实时间步。**特征**：等效于 **LS-DYNA** 或 **Abaqus/Explicit** 等纯显式求解器中的标准单元删除，适用于子弹穿甲、高速爆炸等极度非线性、应力波主导的破碎场景。
  5. 🌊 `Staggered` **(交错显式断裂 - PD社区专属)**：纯显式模式下，内循环收敛并断键后，如果发现拓扑破损，立刻**锁死当前载荷步（不前进）**，原地重启新一轮的显式动力学松弛，直至将断裂释放的冲击动能彻底耗散归零。**特征**：这是近场动力学 (Peridynamics) 社区特有的显式准静态平滑断裂算法，以牺牲显式计算效率的代价换取无震荡的裂纹平稳扩展。
