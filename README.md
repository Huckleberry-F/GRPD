...
7. 🔮 **矩阵免组装纯隐式求解 (Matrix-Free Implicit Solvers)**
   在 V4.5 版本中引入了革命性的 `MatrixFreeImplicitIntegrator` 纯隐式积分器！彻底抛弃显式动力学的伪时间与质量缩放，直接对真实残差进行隐式求根。
   - 核心支持基于 Jacobian-Free Newton-Krylov (**JFNK**) 的无矩阵有限差分 GMRES 求解，赋予系统牛顿法的极速二次收敛性能。
   - 同步支持纯向量点积操作的 Limited-memory BFGS (**L-BFGS**) 优化级求解，极大降低内存占用。在病态塑性硬化和接触问题上展现出无与伦比的计算稳定性与效率！
