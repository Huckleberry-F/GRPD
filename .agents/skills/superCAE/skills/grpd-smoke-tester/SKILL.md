...
description: "GRPD 自动化冒烟测试与验证入口。用于在修改 C++ 源码（如本构模型、积分核）后，一键执行全流程验证：驱动 GRPD 编译求解、调度 ansys-server 提取基准解，并通过 ansys-server.generate_comparison_report 进行数据切片融合与误差图表生成。"
---
