...
void InitContact(PDCommon::Core::PDContext &ctx, const YAML::Node &config) {
  ...
  if (config["ContactPairs"]) {
    ...
    LOG_INFO("[InitContact] No explicit ContactPairs defined in YAML. Contact "
             "engine skipping.");
    return;
  }
  ...
  for (size_t i = 0; i < pairsNode.size(); ++i) {
    ...
    std::string name = node["Name"] ? node["Name"].as<std::string>()
                                    : "ContactPair_" + std::to_string(i);
    std::string type = node["Type"] ? node["Type"].as<std::string>() : "NTN";
    std::string forceLawType =
        node["ForceLaw"] ? node["ForceLaw"].as<std::string>() : "Penalty";
    ...
    if (masterPart == -1 || slavePart == -1) {
      LOG_WARNING("[InitContact] ContactPair '" + name +
                  "' missing MasterPartID or SlavePartID. Skipping.");
      continue;
    }
    ...
    try {
      ...
      for (size_t p = 0; p < particles.size(); ++p) {
        // 我们不在此处筛选 isSurface，而是把整个 Part
        // 都放入，这样能兼容未来的断裂动态表面暴露
      ...
      LOG_ERROR(
          std::string("[InitContact] Failed to create Contact algorithm: ") +
          e.what());
    }
  }
}
