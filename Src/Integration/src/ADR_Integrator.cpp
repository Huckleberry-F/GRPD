...
void ADR_Integrator::run(PDCommon::Core::PDContext &ctx,
    ...
    for (const auto &ta : targets) {
      if (tb.rateField == ta.primaryField && tb.dimension == ta.dimension) {
        ...
        if (uField && vField && aField) {
          soTargets.push_back({uField->dataPtr(), vField->dataPtr(),
  ...
  for (const auto &so : soTargets) {
    if (so.totalComponents > maxTotalComponents)
      maxTotalComponents = so.totalComponents;
  }

  std::vector<std::vector<double>> velHalfOld(
      soTargets.size(), std::vector<double>(maxTotalComponents, 0.0));
  std::vector<std::vector<double>> aOld(
      soTargets.size(), std::vector<double>(maxTotalComponents, 0.0));
  std::vector<std::vector<double>> dispOld(
      soTargets.size(), std::vector<double>(maxTotalComponents, 0.0));

  unsigned long long global_tt_d =
      1; // Tracks the first iteration across the entire explicit session

  for (int step = 1; step <= numLoadSteps_; ++step) {
    LOG_INFO("=== Load Step " + std::to_string(step) + " / " +
             std::to_string(numLoadSteps_) + " ===");
...
#pragma omp parallel for schedule(static)
        ...
        // Kernels will compute F_int and safely convert it to specific
        // acceleration a_new = F_int / m
...
#pragma omp parallel for reduction(- : local_cn1) reduction(+ : local_cn2)     \
    schedule(static)
            for (int i = 0; i < static_cast<int>(so.totalComponents); ++i) {
              ...
              // In GRPD, rigid constraints yield velHalfOld[i] = 0 physically
              // because constraints wipe velocity.
              if (std::abs(velHalfOld[k][i]) > 1e-16) {
                local_cn1 -= so.uPtr[i] * so.uPtr[i] *
                             (so.aPtr[i] - aOld[k][i]) /
                             (dt * velHalfOld[k][i]);
              }
...
#pragma omp parallel for reduction(+ : local_tol2, local_tol3) schedule(static)
        ...
        // Converged if the displacement increment is small AND
        // acceleration residual (force imbalance) is low
        if (TOL2 < dispTol_ && TOL3 < forceTol_ && iter > 20) {
          converged = true;
          LOG_INFO("    [Sub " + std::to_string(sub) +
        ...
        if (iter % 1000 == 0 && iter > 0) {
          LOG_INFO("    > Sub " + std::to_string(sub) + " Iter " +
                   std::to_string(iter) +
      ...
      if (!converged) {
        LOG_WARNING("    [Sub " + std::to_string(sub) +
                    "] Reached MaxPseudoSteps without convergence! TOL2 = " +
                    std::to_string(TOL2_final));
      }

      // [Crucial] Substep is stable, finalize specific state transitions (like
      // plastic strains)
      ...
      // Typically output after every converged substep, using a pseudo-time or
      // load factor as display time
      if (outputCallback) {
