...
## 工具流转
自动化冒烟测试强依赖于 `grpd-server`、`ansys-server` 和 `path-comparison-server` 三个 MCP 服务器的联合：
...
3. **分析模块 (`path-comparison-server`)**：自动滤除无用点云，对齐坐标系，进行插值对比，并生成结果文件夹。

## 快速执行规则
- 不要为冒烟测试临时生成 Python runner、workflow 脚本或批处理脚本。
- 不要扫描全仓库；除非 MCP 返回失败或用户要求排查，否则不读取 MCP 源码。
- 优先直接调用三个 MCP，并把上一步返回的路径传给下一步。
