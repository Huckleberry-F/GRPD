"""
server.py - ANSYS MCP Server 主程�?
启动 MCP 服务器并注册�?AI 使用�?Tools �?Resources�?"""
...
def save_ansys_comparison_to_db(db_path: str, vtk_file: str, ansys_txt_file: str, max_error_uy_percent: float, max_error_seqv_percent: float, output_dir: str, excel_path: str, plot_path: str, zip_path: str):
    ...
    cursor.execute("""
        ...
        (vtk_file, ansys_txt_file, max_error_uy_percent, max_error_seqv_percent, output_dir, excel_path, plot_path, zip_path)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        vtk_file,
        ansys_txt_file,
...
def run_ansys_mac(mac_file: str, work_dir: str = "", job_name: str = "", parameters: dict = None) -> dict:
    """
    运行 ANSYS MAPDL 批处理任务。支持动态向 APDL 注入参数�?    :param mac_file: .mac 脚本文件的绝对路径�?    :param work_dir: 工作目录绝对路径（留空则默认�?mac_file 同目录）�?    :param job_name: 任务名称（留空则默认使用 mac_file 的文件名）�?    :param parameters: 字典形式的参数集，会在执行前�?Wrapper Macro 形式注入 APDL（如 {"START_X": 0.5, "START_Y": 0.0}）�?    """
    if not work_dir:
...
def generate_ansys_apdl_from_yaml(
    ...
    """
    根据 GRPD �?PD.yaml 生成 ANSYS APDL 验证宏，并包�?SAVE 命令生成 .db 文件�?    坐标参数�?APDL 未收到动态参数时使用的默认采样路径�?    """
...
def run_ansys_yaml_case(
    ...
    """
    �?GRPD PD.yaml 生成 APDL，运�?ANSYS，并返回 txt/db/out/err 等结果路径�?    """
    if not work_dir:
    ...
    try:
        ...
        res_dict["db_warning"] = f"写入 SQLite 数据库失�? {str(db_err)}"
...
def get_ansys_solve_status(out_file: str) -> dict:
    """
    解析 ANSYS .out 日志文件，获取求解收敛状态、警告和错误信息�?    :param out_file: .out 文件的绝对路径�?    """
    return ResultParser.parse_out_file(out_file)

@mcp.tool()
def get_ansys_text_results(txt_file: str) -> dict:
    """
    解析 ANSYS PRVAR/PRNSOL 导出的纯文本数据文件 (.txt)�?    返回列名、数据行数、极值以及预览数据，供进一步数值分析�?    :param txt_file: .txt 结果文件的绝对路径�?    """
    return ResultParser.parse_prvar_output(txt_file)

@mcp.tool()
def list_ansys_files(work_dir: str) -> dict:
    """
    列出指定工作目录中的所�?ANSYS 结果文件和日志文件�?    :param work_dir: 工作目录的绝对路径�?    """
...
def generate_comparison_report(
    ...
    """
    读取 GRPD 求解器输出的 VTK 文件�?ANSYS 的文本结果文件，对齐数据并计算相对误差�?    生成 Excel 对比报告以及 PNG 可视化图表，并保存进数据库中�?    :param vtk_file: GRPD 输出的二进制 VTK 文件的绝对路径�?    :param ansys_txt_file: ANSYS 导出�?TXT 结果文件的绝对路径�?    :param output_dir: 生成报告的保存目录（留空则默认自动创建递增编号�?run_XXXX 目录）�?    :param start_x, start_y, start_z: 采样直线起始点坐�?(默认 0.5, 0.0, 0.0)
    :param end_x, end_y, end_z: 采样直线终止点坐�?(默认 0.5, 5.0, 0.0)
    """
    if not output_dir:
        base_work_dir = os.path.join(os.path.dirname(__file__), "work_dir")
    ...
    # 写入 SQLite 数据�?    try:
        ...
        result["db_warning"] = f"写入 SQLite 数据库失�? {str(db_err)}"
